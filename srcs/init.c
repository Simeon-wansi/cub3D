...
void	init_colors(t_game *game)
{
	...
	ft_printf("Floor color initialized to R: %d, G: %d, B: %d\n",
			game->map.floor_color.red, game->map.floor_color.green,
...
//         // Test a few pixels
//         if (game->textures[i].addr && game->textures[i].width > 0
		// && game->textures[i].height > 0)
//         {
//             int test_index = 0;
//             if (test_index < game->textures[i].height
		// * game->textures[i].line_length)
//             {
//                 unsigned char r = (unsigned char)game->textures[i].addr[test_index
	// + 2];
//                 unsigned char g = (unsigned char)game->textures[i].addr[test_index
	// + 1];
